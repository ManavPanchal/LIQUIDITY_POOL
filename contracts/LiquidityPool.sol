...
contract LiquidityPool {
    ...
    function createPool(
        ...
        emit poolCreated(
            owner,
            string.concat(newPool.token1.symbol(), newPool.token2.symbol()),
            block.timestamp
        );
    }
    ...
    function calculateTokenAmount(
        ...
        if (_token == pool[_id].token1) {
            return
                _tokenAmount = (
                    _amount.mul(reserve2).mul(10 ** 18).div(reserve1)
                );
        } else {
            return
                _tokenAmount = (
                    _amount.mul(reserve1).mul(10 ** 18).div(reserve2)
                );
        }
    }

    function addLiquidity(uint _id, uint _amount1, uint _amount2) external {
        uint userToken1Balance = pool[_id].token1.balanceOf(msg.sender);
        uint userToken2Balance = pool[_id].token2.balanceOf(msg.sender);
        require(
            userToken1Balance >= _amount1 && userToken2Balance >= _amount2,
        ...
        if (token1LPT < token2LPT) {
            ...
            providersLPT[_id][msg.sender] = providersLPT[_id][msg.sender].add(
                token1LPT
            );
        } else {
    ...
    function removeLiquidity(uint _id, uint _LPTAmount) external {
        require(
            _LPTAmount <= providersLPT[_id][msg.sender],
            "Insufficient LPT"
        );
        uint reserve1 = pool[_id].balance1;
        uint reserve2 = pool[_id].balance2;
        ...
        providersLPT[_id][msg.sender] = providersLPT[_id][msg.sender].sub(
            _LPTAmount
        );
    ...
    function distributeFee(uint _id, uint _feePerProvider) internal {
        for (uint i = 0; i < liquidityProviders[_id].length; i++) {
            ...
            providersLPT[_id][provider] = providersLPT[_id][provider].add(
                _feePerProvider
            );
        }
    }
