...
const TokenSelector = ({ setTokenSelectorToggle, tokens }) => {
  ...
  const fetchUserTokenBalance = async (tokenAddress) => {
    try {
      const contract = Contract(tokenAddress, tokenABI);
  ...
  const onTokenSelect = async (token) => {
    if (
      tokens.token1.name === token.tokenName ||
      tokens.token2.name === token.tokenName
    )
      return;
  ...
  return (
    <div
      ...
      onClick={() => setTokenSelectorToggle(false)}
    >
      <div
        className="main absolute py sm:top-1/2 left-1/2 bottom-0 -translate-x-1/2 sm:-translate-y-1/2 bg-uni-dim-white w-[400px] max-w-full sm:rounded-3xl rounded-t-3xl max-h-fit overflow-hidden "
        onClick={(e) => {
          e.stopPropagation();
        }}
      >
        <div className="container_header p-5 border-b border-violet-100">
          ...
          <div className="search_bar relative flex items-center gap-1 border border-violet-100 rounded-xl p-1 focus-within:bg-transparent bg-violet-200 bg-opacity-30 w-full ">
            ...
            <input
              type="text"
              className="w-11/12 outline-none bg-transparent text-lg"
        ...
        <div className="token_container pb-1 overflow-y-scroll max-h-[520px]">
          {Tokens.map((token) => {
            return (
              <>
                <div
                  className={`token flex p-2 pl-5 hover:bg-slate-600 hover:bg-opacity-5 cursor-pointer gap-2 items-center ${
                    (tokens.token1.name === token.tokenName ||
                      tokens.token2.name === token.tokenName) &&
                    'opacity-20'
                  }`}
