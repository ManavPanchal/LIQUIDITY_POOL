...
describe('Liquidity Pool Testing', function () {
  ...
  beforeEach(async () => {
    ({ owner, poolInstance, liquidityProvider, swapUser } =
      await deployFixture());
    token1 = await ethers.deployContract('ERC20TestToken');
    token2 = await ethers.deployContract('ERC20TestToken');
    lpt = await ethers.deployContract('ERC20Token', [poolInstance.target]);
  ...
  describe('testing swapping function', () => {
    ...
    it('should revert on invalid pool or token', async () => {
      ...
      await expect(
        poolInstance.swapTokens(0, ethers.parseEther('10'), token3.target),
      ).to.be.revertedWith('invalid pool id or token address');
    });

    it("should revert if pool is at it's min balance limit", async () => {
      await expect(
        poolInstance.swapTokens(0, ethers.parseEther('10'), token2.target),
      ).to.be.revertedWith('Exceeds Min Balance');
    });
    ...
    it('should transfer expected value to user', async () => {
      ...
      const swappingDetails = await poolInstance.calculateSwappingAmount(
        0,
      ...
      await token1
        .connect(liquidityProvider)
        .approve(poolInstance.target, ethers.parseEther('10'));
      await poolInstance
        .connect(liquidityProvider)
        .swapTokens(0, ethers.parseEther('10'), token1.target);
      const updatedBalance = await token2.balanceOf(liquidityProvider.address);
      expect(Number(updatedBalance)).to.be.eq(
        Number(token2Balance + swappingDetails[0]),
      );
    });
  });
  ...
  describe('testing remove liquidity function', () => {
    ...
    it('should revert on insufficient amount od LPT', async () => {
      await expect(
        poolInstance
          .connect(liquidityProvider)
          .removeLiquidity(0, ethers.parseEther('10')),
      ).to.be.revertedWith('Insufficient LPT');
    });
    ...
    it('should burn LPT on removing liquidity', async () => {
      ...
      expect(await lpt.totalSupply()).to.be.equal(
        totalSupply - ethers.parseEther('10'),
      );
    });
    ...
    it('should update the balance of user on remove liquidity', async () => {
      ...
      await lpt.approve(poolInstance.target, ethers.parseEther('10'));
      await poolInstance.removeLiquidity(0, ethers.parseEther('10'));
      expect(await lpt.balanceOf(owner.address)).to.be.equal(
        userLPTbalance - ethers.parseEther('10'),
      );
    });
  });
});
